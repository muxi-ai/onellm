...
def long_context_example():
    ...
    millions of years. This led to rapid advances in medicine, climate modeling, and space exploration.
    
    Dr. Sarah Chen was at the forefront of this revolution. Her team at the Quantum Research Institute 
    had developed the first stable quantum computer that could operate at room temperature. This 
    eliminated the need for expensive cooling systems and made quantum computing accessible to 
    ...
    response = client.chat.completions.create(
        model="xai/grok-2-latest",
        messages=[
            {"role": "user", "content": f"Based on this story, what were the three most important breakthroughs mentioned?\n\nStory: {long_context}"}
        ],
        max_tokens=200
    )
        ...
        messages=[
            {
                ...
                can we conclude that some Bloops are not Blips? Explain your reasoning step by step."""
            }
        ],
        temperature=0.3,
        max_tokens=300
...
def creative_writing_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a creative writer with a unique perspective."
            },
            {
                "role": "user",
                "content": "Write the opening paragraph of a science fiction story set on Mars in the year 2150."
            }
        ],
        temperature=0.9,
        max_tokens=200
...
def code_generation_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Write a Python function that implements a binary search tree with insert, search, and delete operations."
            }
        ],
        temperature=0.2,
        max_tokens=500
