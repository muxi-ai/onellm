    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "What are the advantages of using OpenRouter for accessing multiple LLMs?",  # noqa: E501
                }
            ],
            max_tokens=150,
        )
...
def json_mode_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": "List 3 benefits of using OpenRouter in JSON format."},
        ],
        max_tokens=200,
    )
...
def multi_language_example():
    ...
    for lang, text in languages:
        response = client.chat.completions.create(
            model="openrouter/meta-llama/llama-3.2-3b-instruct:free",
            messages=[
                {"role": "user", "content": f"Translate '{text}' and respond in the same language."},  # noqa: E501
            ],
            max_tokens=50,
        )
        print(f"\n{lang}: {text}")
