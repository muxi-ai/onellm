...
def function_calling_example():
    ...
    response = client.chat.completions.create(
        model="mistral/mistral-small-latest",
        messages=[{"role": "user", "content": "How many euros would I get for 100 US dollars?"}],
        tools=tools,
        tool_choice="auto",
    )
...
def code_generation_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Write a Python function to merge two sorted lists into one sorted list.",  # noqa: E501
            }
        ],
        temperature=0.2,
        max_tokens=300,
...
def multilingual_example():
    ...
    for language, greeting in languages:
        response = client.chat.completions.create(
            model="mistral/mistral-small-latest",
            messages=[
                {"role": "user", "content": f"Respond to this greeting appropriately: {greeting}"}
            ],
            max_tokens=50,
        )
        print(f"\n{language}: {greeting}")
