...
def json_mode_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": "Create a JSON object with information about 3 Google products and their main features.",  # noqa: E501
            },
        ],
...
def long_context_example():
    ...
    response = client.chat.completions.create(
        model="google/gemini-1.5-flash",
        messages=[{"role": "user", "content": f"Summarize this text in 2 sentences: {long_text}"}],
        max_tokens=100,
    )
...
def safety_settings_example():
    ...
    response = client.chat.completions.create(
        model="google/gemini-1.5-flash",
        messages=[
            {"role": "user", "content": "Write a children's story about a brave little robot."}
        ],
        temperature=0.9,
        max_tokens=200,
    )
...
def code_generation_example():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Write a Python class for a simple task queue with add, remove, and process methods.",  # noqa: E501
            }
        ],
        temperature=0.2,
        max_tokens=400,
