...
class LlamaCppProvider(Provider):
    ...
    def _parse_model_path(self, model: str) -> Path:
        ...
        if model.startswith("/") or (len(model) > 1 and model[1] == ":"):  # Unix or Windows absolute path
    ...
    def _load_model(self, model_path: Path, **kwargs) -> Any:
        ...
        if model_key in _MODEL_CACHE:
            ...
            if (cached_model.n_ctx == kwargs.get("n_ctx", self.n_ctx) and
                cached_model.n_gpu_layers == kwargs.get("n_gpu_layers", self.n_gpu_layers)):
                return cached_model
            else:
    ...
    async def download_file(self, file_id: str, **kwargs) -> bytes:
        ...
        """
        raise InvalidRequestError(
            "llama.cpp provider does not support file downloads.",
            provider="llama_cpp"
        )
