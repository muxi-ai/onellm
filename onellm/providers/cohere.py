...
class CohereProvider(Provider):
    ...
    async def _make_request(
        ...
        client, pooled = await get_session_safe("cohere")
        if stream:
            # Stream lifecycle is owned by the generator; close the
            # ad-hoc client inside its ``finally``.
            return self._stream(client, pooled, request_kwargs)
        try:
            response = await client.request(**request_kwargs)
            if response.status_code not in (200, 201):
                # Tolerant read normalizes non-JSON error bodies into
                # the ``{"error": {"message": ...}}`` shape that
    ...
    async def _stream(
        ...
        """Open a streaming request and yield parsed Cohere JSONL chunks.

        Cohere's streaming format is one JSON object per line (no
        ``data:`` prefix, no ``[DONE]`` sentinel) so the parser is
        simpler than the SSE-style providers.
        """
        try:
