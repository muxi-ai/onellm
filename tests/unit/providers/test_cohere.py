...
class TestCohereProvider:
    ...
    async def test_create_embedding(self, provider):
        ...
        response = await provider.create_embedding(
            model="cohere/embed-english-v3.0", input="Hello, world!"
        )
    ...
    async def test_multiple_cohere_models(self, provider):
        ...
        chat_models = ["cohere/command-r", "cohere/command-r-plus", "cohere/command"]
    ...
    async def test_embedding_models(self, provider):
        ...
        embedding_models = ["cohere/embed-english-v3.0", "cohere/embed-multilingual-v3.0"]

        for model in embedding_models:
            try:
                response = await provider.create_embedding(model=model, input="Test text")
                assert response is not None
                assert len(response.data) == 1
                assert isinstance(response.data[0].embedding, list)
            except InvalidRequestError as e:
    ...
    async def test_batch_embeddings(self, provider):
        ...
        response = await provider.create_embedding(model="cohere/embed-english-v3.0", input=texts)
