...
@pytest.mark.skipif(
    not os.path.exists("vertexai.json"),
    reason="vertexai.json credentials file not found"
)
    ...
    async def test_multiple_gemini_models(self, provider):
        ...
        for model in models:
            try:
                ...
                if "model not found" not in str(e).lower() and "not supported" not in str(e).lower():
                    raise
    ...
    async def test_vision_capability(self, provider):
        ...
        response = await provider.create_chat_completion(
            ...
            messages=[{
                "role": "user",
                "content": [
                    {"type": "text", "text": "Describe what you would see in a typical beach scene"},
                ]
            }],
            max_tokens=30
        )
